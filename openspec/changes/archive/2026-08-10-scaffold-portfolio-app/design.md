...
| `production.py` | `DEBUG=False`, `ALLOWED_HOSTS=env.list(...)`, `DATABASES={"default": env.db("DATABASE_URL")}`, `CORS_ALLOWED_ORIGINS=env.list(...)`, `SECURE_SSL_REDIRECT`, HSTS, `SECURE_PROXY_SSL_HEADER`, `CSRF_TRUSTED_ORIGINS`. |
